...
# To restart it:
#
#     sudo kill $(ps aux | grep --regexp "botListener.sh" | awk '{print $2}') ; sudo /opt/telegramMonitoring/botListener.sh &
#
...
echo -e "<code>%F0%9F%91%8D\nListener is ON. Use </code>/help<code> to see the available commands</code>" | $my_path/sendTelegramMessage.sh
...
do
	...
	FNAME=$(echo $CURLOUTPUT | jq '.result[0].message.from.first_name')
	LNAME=$(echo $CURLOUTPUT | jq '.result[0].message.from.last_name')
	USERNAME=$(echo $CURLOUTPUT | jq '.result[0].message.from.username')

	# If the message ID has changed...
	if [ $LASTMESSAGE_ID != $MESSAGE_ID ]
	then

		# If it comes from the Telegram user account...
		if [ $FROM = $telegramId ]
		then
			...
			then
				...
				for filename in $(ls -1 --almost-all "$my_path/botCommands");
				do
					if ( getfacl $my_path/botCommands/$filename --omit-header --absolute-names | grep "user::" | grep "x" > /dev/null 2>&1 )
					then
						HELPTEXT=$(grep --ignore-case '^#HELP:' "$my_path/botCommands/$filename" | cut --delimiter=':' --fields=2 | xargs)
						printf -v PAD %30s
			...
			OUTPUT=$( \
				echo "%F0%9F%9B%8E" ; \
				echo "Another user is trying to talk with me." ; \
		...
		CMD="$my_path/botCommands/$(echo $TEXT | tr --delete '/' | tr --delete '"').sh"
		if [[ -x "$CMD" ]]
		then
			OUTPUT=$($CMD)
		fi
		# Send output via Telegram
		echo "<code>$OUTPUT</code>" | $my_path/sendTelegramMessage.sh
